...
| Worst Nodes | 12 | 12 |
| Average Time | 2.1325 µs | 2.2935 µs |
| Worst-case Time | 3.4842 µs | 4.1304 µs |
...
BFS and DFS were tested using the same 12-node graph and the same set of goal nodes. The average number of explored nodes was 6.5 for both algorithms. In this experiment, BFS recorded an average execution time of 2.1325 microseconds, while DFS recorded 2.2935 microseconds. Therefore, BFS showed a slightly lower measured execution time for this particular test. The result depends on the graph structure and test cases used. For larger graphs, the number of nodes explored and execution time can increase significantly.
